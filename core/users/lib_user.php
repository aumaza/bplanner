...
// SETTERS
	...
	private function setCelular($var){
		$this->avatar = $var;
	}
...
// GETTERS
	...
	private function getCelular($var){
...
public function formUserInfo($oneUser,$user_id,$conn,$db_basename){
	...
	$sql = "select * from bp_usuarios where id = '$user_id'";
	$query = mysqli_query($conn,$sql);
	...
	echo '<div class="container">
				   ...
				   <form id="fr_update_user_ajax" method="POST" enctype="multipart/form-data">
				    ...
				    <div class="form-group">
				      <label for="movil">Celular / Móvil:</label>
				      <input type="text" class="form-control" id="celular" name="celular" maxlength="10" placeholder="Ingrese su Número de Celular. Ej.: 1166667777" value="'.$oneUser->getCelular($row['celular']).'">
				    </div>
				    ...
				    <div class="alert alert-warning">
				     ...
				     <div class="form-group">
					  ...
					  <select class="form-control" id="tasks" name="tasks">
					    ...
					    <option value="Data_Analitics">Data Analitics</option>
					  </select>
					</div>
...
*/
public function updateUserInfo($oneUser,$id,$celular,$task,$file,$conn,$db_basename){
		...
		$allowTypes = array('png','jpg');
		    ...
		    // Upload file to server
		        if(move_uploaded_file($_FILES["my_file"]["tmp_name"], $targetFilePath)){
		        	...
		        	mysqli_select_db($conn,$db_basename);
					$sql = "update bp_usuarios set celular = $oneUser->setCelular('$celular'), functions = CONCAT_WS(',',functions,$oneUser->setTask('$task')), avatar = $oneUser->setAvatar('$targetFilePath') where id = '$id'";
	...
	if($file == ''){
		...
		mysqli_select_db($conn,$db_basename);
		$sql = "update bp_usuarios set celular = $oneUser->setCelular('$celular'), functions = CONCAT_WS(',',functions,$oneUser->setTask('$task')) where id = '$id'";
...
public function updatePassword($oneUser,$id,$password_actual,$password_1,$password_2,$conn,$db_basename){
	...
	mysqli_select_db($conn,$db_basename);
	$sql = "select password from bp_usuarios where id = '$id'";
	$query = mysqli_query($conn,$sql);
	while($row = mysqli_fetch_array($query)){
		...
		if(password_verify($password_actual, $hash)){
				...
				if($oneUser->validatePassword($password_1,$password_2) == 0){
					...
					$sql_1 = "update bp_usuarios set password = $oneUser->setPassword('$passHash') where id = '$id'";
