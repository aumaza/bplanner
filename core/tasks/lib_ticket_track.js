...
// ACTUALIZAR REGISTRO DE AVANCE DE TICKET
$(document).ready(function(){
    $('#update_advance').click(function(){
        
        const form = document.querySelector('#fr_update_advance_ticket_ajax');
        ...
        Array.from(files).forEach(file => {
         ...
         jQuery.ajax({
            ...
            success:function(r){
                if(r == 1){
                    var mensaje = `<br><div class="alert alert-success alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                        <p align=center><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> Avance Actualizado Exitosamente</p></div>`;
                        document.getElementById('messageUpdateTrack').innerHTML = mensaje;
                         $('#advance').disabled = true;
                         $('#cant_hours').disabled = true;
                         $('#state').disabled = true;
                         $('#files').disabled = true;
                        ...
                        var form = $('<form action="#" method="post">' +
                          '<input type="hidden" name="id" value="'+id_ticket.value+'" />' +
                          '<input type="hidden" name="forwards" />' +
                          '</form>');
                        $('body').append(form);
                        form.submit();
                    }
                        ...
                        var mensaje = `<br><div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                        <p align=center><span class="glyphicon glyphicon-alert" aria-hidden="true"></span> Ocurrió un problema al intentar actualizar el Avance</p></div>`;
                        document.getElementById('messageUpdateTrack').innerHTML = mensaje;
                        console.log(formData);
                        setTimeout(function() { $(".close").click(); }, 4000);
                    }
